    ...
    public void analyzeAndApproveListingAsync(Listing listing) {
        CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                parts.add(Map.of("text", "Bạn là một kiểm duyệt viên nền tảng cho thuê phòng trọ. Hãy phân tích các hình ảnh và mô tả sau. Trả lời 'YES' nếu hình ảnh thực sự là phòng trọ/nhà ở hợp lệ, sạch sẽ và không chứa nội dung nhạy cảm hay quảng cáo rác. Trả lời 'NO' nếu hình ảnh không hợp lệ (không phải là phòng, ảnh selfie, v.v.). Chỉ trả lời YES hoặc NO. Mô tả phòng: " + (listing.getDescription() != null ? listing.getDescription() : "")));
                ...
                Map<String, Object> requestBody = Map.of(
                        ...
                        "generationConfig", Map.of("temperature", 0.1, "maxOutputTokens", 10)
                );
                ...
                if (body != null && body.containsKey("candidates")) {
                    ...
                    if (!candidates.isEmpty()) {
                        ...
                        if (!resParts.isEmpty()) {
                            ...
                            if (text != null && text.trim().toUpperCase().contains("YES")) {
                                isApproved = true;
                            }
                        }
                    }
                ...
                System.out.println("[GEMINI AI] Kết quả kiểm duyệt: " + (isApproved ? "ĐẠT CHUẨN (YES) -> Đã tự động duyệt" : "TỪ CHỐI (NO) -> Chuyển sang chờ duyệt tay"));
